...
- **Attack player (PvP)** (see `.../docs/game-api/pvp.md`): `POST somethingelse.php?attackid={playerId}&r={redir}` `message&rage&hash` → 302 `/plrattack/{id}/`; a **200 = refused**. `rage` is the **server-supplied cost** (hidden field, not a 2–50 slider); `hash` comes from any list rendering `showAttackWindow(name,id,rage,hash[,redir])`. **One attack per target per 60 min** ("You can only attack someone once every 60 minutes, and you attacked this person {n} minutes ago" — parse `n` to reschedule); Time Warp (skill 3017, Affliction) is a *per-character* exception. Target lists: `playersearch.php` (`searchType` 0=begins-with, 1=contains), `/myhitlist`, `/crew_hitlist` (404 hashed targets in one GET), `crew_profile.php?id={crewId}` (any crew's roster, no hashes), `/closedpvp[?type=1]` brawls. Level cells colour-code attackability: `#00FFFF` in range, `#00FF00` too weak, `#FF0000` too powerful. `/attacklog?mode=out` = authoritative cooldown state. **Game clock = UTC−5, no DST.**
...
- **Teleports** (see `.../docs/game-api/teleports.md`): item = `POST ajax/backpack_action.php` `action=activate&itemids[]={iid}` (key-tab items whose `makemenu` flags contain `a`) → free, no cooldown, **not consumed**; skill 27 = `POST cast_skills` `dest={roomId}&castskillid=27&cast=Cast Skill` (100 rage/60m, `dest` list only in `skills_info.php?id=27`); home tavern = `world.php?teleport=1`, re-homed via `?teleportupdate=1&tavern={roomId}`. Landing room always comes from a plain room reload. **Availability is per character** (level/quest progress) — never hardcode.
